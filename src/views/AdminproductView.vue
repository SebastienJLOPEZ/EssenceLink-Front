      ...
      <div class="content">
        ...
        <div v-if="currentTab === 'productList'">
          ...
          <div v-for="(product, index) in productList" :key="index" class="product-card">
            ...
            <button @click="openModifyPopup(product)">Modify</button>
          </div>
          <div v-if="showModifyPopup" class="popup">
            <div class="popup-content">
              <span class="close-popup" @click="closeModifyPopup">×</span>
              <h2 class="popup-title">Modify Product</h2>
              <div class="popup-details">
                <label for="modifiedDescription">Modified Description:</label>
                <input type="text" v-model="modifiedProduct.description" id="modifiedDescription" />
              </div>
              <div class="popup-details">
                <label for="modifiedPrice">Modified Price:</label>
                <input type="number" v-model="modifiedProduct.price" id="modifiedPrice" />
              </div>
              <div class="popup-details">
                <label for="modifiedStock">Modified Stock:</label>
                <input type="number" v-model="modifiedProduct.stock" id="modifiedStock" />
              </div>
              <div class="buttons">
                <button @click="saveModifiedProduct" class="save-changes">Save Changes</button>
              </div>
            </div>
          </div>
        </div>
